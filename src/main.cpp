...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        window.setMouseButtonCallback([&eventBus, &inputState](int button, int action, int mods, double xpos, double ypos) {
        ...
        eventBus.Subscribe(Renderer::EventType::Key, 10, [&](Renderer::Event& evt) {
            ...
            if (guiLayer.WantCaptureKeyboard()) return;
            bool pressed = e.action != ACTION_RELEASE;
            if (e.key == static_cast<int>(Renderer::Key::W)) inputState.moveForward = pressed;
            if (e.key == static_cast<int>(Renderer::Key::S)) inputState.moveBackward = pressed;
            if (e.key == static_cast<int>(Renderer::Key::A)) inputState.moveLeft = pressed;
            if (e.key == static_cast<int>(Renderer::Key::D)) inputState.moveRight = pressed;
            if (e.key == static_cast<int>(Renderer::Key::Q)) inputState.moveDown = pressed;
            if (e.key == static_cast<int>(Renderer::Key::E)) inputState.moveUp = pressed;
            if (e.key == static_cast<int>(Renderer::Key::Escape) && pressed) inputState.exitRequested = true;
        ...
        eventBus.Subscribe(Renderer::EventType::MouseMove, 10, [&](Renderer::Event& evt) {
            ...
            if (!inputState.rightMouseDown) return;
        ...
        while (!window.shouldClose()) {
            ...
            else
            {
                ...
                lightingPass.Render(
            geometryPass.getPositionTexture(),
                 geometryPass.getNormalTexture(), 
                 geometryPass.getDiffuseTexture(), 
                 geometryPass.getSpecularTexture(), 
