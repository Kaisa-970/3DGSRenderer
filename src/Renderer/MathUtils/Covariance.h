...
public:
    // 从缩放和旋转四元数计算3D协方差矩阵
    // 返回3x3矩阵（行主序存储在9个元素中）
    static void compute3DCovariance(const FLOAT *scale,    // [sx, sy, sz]
    ...
    // 将3D协方差投影到2D屏幕空间
    // 这是简化版本，完整版需要雅可比矩阵
    static void project3DCovariance(const FLOAT *cov3D,             // 3x3输入协方差
                                    ...
                                    FLOAT *cov2D)                   // 输出：2x2协方差矩阵
    {
        ...
        // 提取视图矩阵的旋转部分（3x3）
        FLOAT W[9] = {viewMatrix[0], viewMatrix[1], viewMatrix[2], viewMatrix[4], viewMatrix[5],
                      viewMatrix[6], viewMatrix[8], viewMatrix[9], viewMatrix[10]};
        ...
        //     [ 0,  fy/z ]
        //     [-fx*x/z², -fy*y/z²]
        FLOAT J_full[6] = {focal_x / limz, 0, -(focal_x * t_x) / limz2, 0, focal_y / limz, -(focal_y * t_y) / limz2};
