...
void Camera::getViewMatrix(float* matrix) const {
    ...
    // 转换为列主序
    matrix[0] = view.m[0]; matrix[4] = view.m[1]; matrix[8] = view.m[2];   matrix[12] = view.m[3];
    matrix[1] = view.m[4]; matrix[5] = view.m[5]; matrix[9] = view.m[6];   matrix[13] = view.m[7];
    matrix[2] = view.m[8]; matrix[6] = view.m[9]; matrix[10] = view.m[10]; matrix[14] = view.m[11];
    matrix[3] = view.m[12]; matrix[7] = view.m[13]; matrix[11] = view.m[14]; matrix[15] = view.m[15];
}
...
void Camera::getPerspectiveMatrix(float* matrix, float fov, float aspect, float near, float far) const {
    ...
    // 转换为列主序
    matrix[0] = proj.m[0]; matrix[4] = proj.m[1]; matrix[8] = proj.m[2];   matrix[12] = proj.m[3];
    matrix[1] = proj.m[4]; matrix[5] = proj.m[5]; matrix[9] = proj.m[6];   matrix[13] = proj.m[7];
    matrix[2] = proj.m[8]; matrix[6] = proj.m[9]; matrix[10] = proj.m[10]; matrix[14] = proj.m[11];
    matrix[3] = proj.m[12]; matrix[7] = proj.m[13]; matrix[11] = proj.m[14]; matrix[15] = proj.m[15];
}

void Camera::getOrthographicMatrix(float* matrix, float left, float right, float bottom, float top, float near, float far) const {
    Matrix4 ortho = getOrthographicMatrix(left, right, bottom, top, near, far);
    
    // 转换为列主序
    matrix[0] = ortho.m[0]; matrix[4] = ortho.m[1]; matrix[8] = ortho.m[2];   matrix[12] = ortho.m[3];
    matrix[1] = ortho.m[4]; matrix[5] = ortho.m[5]; matrix[9] = ortho.m[6];   matrix[13] = ortho.m[7];
    matrix[2] = ortho.m[8]; matrix[6] = ortho.m[9]; matrix[10] = ortho.m[10]; matrix[14] = ortho.m[11];
    matrix[3] = ortho.m[12]; matrix[7] = ortho.m[13]; matrix[11] = ortho.m[14]; matrix[15] = ortho.m[15];
}
