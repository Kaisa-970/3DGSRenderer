...
public:
    Material();
    Material(const std::string& name, const Vector3& diffuseColor = Vector3::ZERO, const Vector3& specularColor = Vector3::ONE, const Vector3& ambientColor = Vector3::ZERO, float shininess = 32.0f);
    ~Material();
    ...
    const std::vector<std::shared_ptr<Texture2D>>& getDiffuseTextures() const { return m_diffuseTextures; }
