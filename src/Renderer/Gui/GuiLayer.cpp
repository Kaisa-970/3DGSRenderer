    ...
    if (auto selected = selected_.lock()) {
        ...
        if (selected->getType() == RenderableType::Primitive) {
            ...
            if (mat) {
                ...
                ImGui::Text("Diffuse: (%.3f, %.3f, %.3f)", mat->getDiffuseColor().x, mat->getDiffuseColor().y, mat->getDiffuseColor().z);
                ImGui::Text("Specular: (%.3f, %.3f, %.3f)", mat->getSpecularColor().x, mat->getSpecularColor().y, mat->getSpecularColor().z);
                ImGui::Text("Ambient: (%.3f, %.3f, %.3f)", mat->getAmbientColor().x, mat->getAmbientColor().y, mat->getAmbientColor().z);
                float shininess = mat->getShininess();
                if (ImGui::SliderFloat("Shininess", &shininess, 1.0f, 256.0f, "%.1f")) {
                    mat->setShininess(shininess);
                }
                ImGui::Text("Textures: diffuse %zu, specular %zu, normal %zu",
                    mat->getDiffuseTextures().size(),
            ...
            if (model) {
                ...
                if (!submeshes.empty() && submeshes[0].material) {
                    ...
                    ImGui::Text("Material[0] Diffuse: (%.3f, %.3f, %.3f)", mat->getDiffuseColor().x, mat->getDiffuseColor().y, mat->getDiffuseColor().z);
                    ImGui::Text("Material[0] Specular: (%.3f, %.3f, %.3f)", mat->getSpecularColor().x, mat->getSpecularColor().y, mat->getSpecularColor().z);
                    float shininess = mat->getShininess();
        ...
        if (gbufferViewMode_ && !gbufferViewLabels_.empty()) {
            ...
            ImGui::Combo("Texture", gbufferViewMode_, gbufferViewLabels_.data(), static_cast<int>(gbufferViewLabels_.size()));
        }
        ImGui::End();
    }
}
...
void GuiLayer::SyncEditableFromTransform(const Renderable& renderable) {
    ...
    // 防止零尺度
    if (editScale_.x == 0) editScale_.x = 1.0f;
    if (editScale_.y == 0) editScale_.y = 1.0f;
    if (editScale_.z == 0) editScale_.z = 1.0f;
    ...
    (void)r01; (void)r02; (void)r11; (void)r12; // 当前欧拉提取未用到其余分量
...
void GuiLayer::ApplyEditableToRenderable(Renderable& renderable) {
    // 以顺序: Scale -> RotX -> RotY -> RotZ -> Translate，然后转置以匹配列主序存储
