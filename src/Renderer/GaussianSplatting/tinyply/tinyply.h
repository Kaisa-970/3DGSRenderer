...
static std::map<Type, PropertyInfo> PropertyTable{
    {Type::INT8, PropertyInfo(1, std::string("char"))},      {Type::UINT8, PropertyInfo(1, std::string("uchar"))},
    {Type::INT16, PropertyInfo(2, std::string("short"))},    {Type::UINT16, PropertyInfo(2, std::string("ushort"))},
    {Type::INT32, PropertyInfo(4, std::string("int"))},      {Type::UINT32, PropertyInfo(4, std::string("uint"))},
    {Type::FLOAT32, PropertyInfo(4, std::string("float"))},  {Type::FLOAT64, PropertyInfo(8, std::string("double"))},
...
struct PlyFile
{
    ...
    void add_properties_to_element(const std::string &elementKey, const std::vector<std::string> propertyKeys,
...
inline uint64_t endian_swap<uint64_t, uint64_t>(const uint64_t &v) noexcept
{
    return (((v & 0x00000000000000ffLL) << 56) | ((v & 0x000000000000ff00LL) << 40) |
            ((v & 0x0000000000ff0000LL) << 24) | ((v & 0x00000000ff000000LL) << 8) | ((v & 0x000000ff00000000LL) >> 8) |
            ((v & 0x0000ff0000000000LL) >> 24) | ((v & 0x00ff000000000000LL) >> 40) |
...
struct PlyFile::PlyFileImpl
{
    ...
    size_t read_property_binary(const size_t &stride, void *dest, size_t &destOffset, size_t destSize,
                                std::istream &is);
    size_t read_property_ascii(const Type &t, const size_t &stride, void *dest, size_t &destOffset, size_t destSize,
                               std::istream &is);
...
size_t PlyFile::PlyFileImpl::read_property_binary(const size_t &stride, void *dest, size_t &destOffset, size_t destSize,
                                                  std::istream &is)
{
...
size_t PlyFile::PlyFileImpl::read_property_ascii(const Type &t, const size_t &stride, void *dest, size_t &destOffset,
                                                 size_t destSize, std::istream &is)
{
    if (destOffset + stride > destSize)
    {
        throw std::runtime_error("unexpected EOF. malformed file?");
...
void PlyFile::PlyFileImpl::write_property_binary(std::ostream &os, const uint8_t *src, size_t &srcOffset,
        ...
        for (size_t i = 0; i < e.size; ++i)
        {
            ...
            for (auto &p : e.properties)
            {
                ...
                if (p.isList)
                {
                    ...
                    write_property_binary(os, (helper->data->buffer.get_const() + helper->cursor->byteOffset),
    ...
    for (auto &e : elements)
    {
        ...
        for (const auto &p : e.properties)
        {
            ...
            if (!lookup.skip)
            {
                if (p.isList)
                {
                    os << "property list " << PropertyTable[p.listType].str << " " << PropertyTable[p.propertyType].str
                       << " " << p.name << "\n";
                }
                else
                {
...
std::shared_ptr<PlyData> PlyFile::PlyFileImpl::request_properties_from_element(
    const std::string &elementKey, const std::vector<std::string> propertyKeys, const uint32_t list_size_hint)
{
    if (elements.empty())
    ...
    if (elementIndex >= 0)
    {
        ...
        for (const auto &key : propertyKeys)
        {
            ...
            auto result =
                userData.insert(std::pair<uint32_t, ParsingHelper>(hash_fnv1a(element.name + property.name), helper));
            if (result.second == false)
            {
                throw std::invalid_argument("element-property key has already been requested: " + element.name + " " +
                                            property.name);
            }
        }
        ...
        if (std::adjacent_find(propertyTypes.begin(), propertyTypes.end(), std::not_equal_to<Type>()) !=
            propertyTypes.end())
        {
...
void PlyFile::PlyFileImpl::add_properties_to_element(const std::string &elementKey,
                                                     const std::vector<std::string> propertyKeys, const Type type,
                                                     const size_t count, const uint8_t *data, const Type listType,
                                                     const size_t listCount)
{
    ParsingHelper helper;
    ...
    auto create_property_on_element = [&](PlyElement &e) {
        ...
        {
            PlyProperty newProp =
                (listType == Type::INVALID) ? PlyProperty(type, key) : PlyProperty(listType, type, key, listCount);
    ...
    else
    {
        PlyElement newElement =
            (listType == Type::INVALID) ? PlyElement(elementKey, count) : PlyElement(elementKey, count);
...
{
    std::function<void(PropertyLookup & f, const PlyProperty &p, uint8_t *dest, size_t &destOffset, size_t destSize,
                       std::istream &is)>
        read;
    ...
    auto read_list_binary = [this](const Type &t, void *dst, size_t &destOffset, const size_t &stride,
    ...
    if (isBinary)
    {
        read = [this, &listSize, &dummyCount, &read_list_binary](PropertyLookup &f, const PlyProperty &p, uint8_t *dest,
            ...
            return read_property_binary(f.prop_stride * listSize, dest + destOffset, destOffset, destSize,
    ...
    else
    {
        read = [this, &listSize, &dummyCount](PropertyLookup &f, const PlyProperty &p, uint8_t *dest,
            ...
            {
                dummyCount = 0;
                read_property_ascii(p.listType, f.list_stride, &listSize, dummyCount, sizeof(listSize),
                                    _is); // the list size
...
bool PlyFile::is_binary_file() const
{
...
void PlyFile::add_properties_to_element(const std::string &elementKey, const std::vector<std::string> propertyKeys,
                                        const Type type, const size_t count, const uint8_t *data, const Type listType,
                                        const size_t listCount)
{
