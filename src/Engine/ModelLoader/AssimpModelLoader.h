...
public:
    ...
    // 获取加载统计信息
    size_t getLoadedVertices() const
    {
        return totalVertices_;
    }
    size_t getLoadedFaces() const
    {
        return totalFaces_;
    }
    size_t getLoadedMeshes() const
    {
        return totalMeshes_;
    }
...
private:
    ...
    // 处理单个网格
    void processMesh(aiMesh *mesh, const aiScene *scene, Renderer::Mesh &outMesh, const Renderer::Mat4 &transform);

    // 处理材质
    void processMaterial(aiMaterial *material, const aiScene *scene, const std::string &directory,
                         std::shared_ptr<Renderer::Material> &outMaterial);

    // 处理节点（递归）
    void processNode(aiNode *node, const aiScene *scene, std::vector<Renderer::SubMesh> &subMeshes,
                     const Renderer::Mat4 &parentTransform,
    ...
    // 转换Assimp矩阵到我们的矩阵类型
    Renderer::Mat4 assimpToMatrix4(const aiMatrix4x4 &m);

    // // 应用变换到顶点列表
    // void applyTransformToVertices(std::vector<Vertex>& vertices, const Mat4& transform);
